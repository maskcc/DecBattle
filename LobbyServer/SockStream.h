...
public:
    SockStream();
    int32_t reciveMsg(int32_t fd, MsgBase* &msg);
    int32_t sendMsg(int32_t fd, MsgBase* &msg);
    int32_t reset();
    virtual ~SockStream();
protected:
    ...
    //char m_stream[BUFF_LENGTH];  //最大缓冲区, 8KB, 改成4KB //改成弹性的vector<char>比较好
    char m_headBuf[sizeof(MsgHeadDef)];
